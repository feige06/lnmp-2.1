...
Install_Database()
{
    ...
    if [[ "${DBSelect}" =~ ^[1-5]$ ]]; then
        if [[ "${Bin}" = "y" && "${DBSelect}" =~ ^[1-3]$ ]]; then
            ...
            [[ $? -ne 0 ]] && Download_Files https://cdn.mysql.com/archives/mysql-${Mysql_Ver_Short}/${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz
            if [ ! -s ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz ]; then
            ...
            [[ $? -ne 0 ]] && Download_Files https://cdn.mysql.com/archives/mysql-8.0/${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz
            if [ ! -s ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz ]; then
            ...
            [[ $? -ne 0 ]] && Download_Files https://cdn.mysql.com/archives/mysql-8.4/${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz ${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz
            if [ ! -s ${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz ]; then
...
Install_Only_Database()
{
    clear
    echo "+-----------------------------------------------------------------------+"
    echo "|      Install MySQL database for LNMP, Written by Licess       |"
    echo "+-----------------------------------------------------------------------+"
    echo "|               A tool to install MySQL for LNMP                |"
    echo "+-----------------------------------------------------------------------+"
    echo "|           For more information please visit https://lnmp.org          |"
    echo "+-----------------------------------------------------------------------+"
    ...
    Echo_Red "The script will REMOVE MySQL installed via yum or apt-get and its databases!!!"
    Press_Install
