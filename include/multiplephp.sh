...
#which PHP Version do you want to install?
    ...
    if [ -x /usr/local/php/bin/php-config ]; then
        Main_PHP_Version=$(/usr/local/php/bin/php-config --version)
        Main_PHP_Version=${Main_PHP_Version%.*}
    fi
    Echo_Yellow "You have 15 options for your PHP install."
    for i in {1..15}; do
        MPHP_Version=${PHP_Info[$((i-1))]#PHP }
        MPHP_Path="/usr/local/php${MPHP_Version%.*}"
        if [[ -d "${MPHP_Path}" || "${MPHP_Version%.*}" = "${Main_PHP_Version}" ]]; then
            echo "${i}: Install ${PHP_Info[$((i-1))]}（已安装）"
        else
            echo "${i}: Install ${PHP_Info[$((i-1))]}"
        fi
    done
    read -p "Enter your choice (1-15): " PHPSelect

    if [[ ! "${PHPSelect}" =~ ^([1-9]|1[0-5])$ ]]; then
        echo "No enter,You Must enter one option."
        exit 1
    fi
    echo "You will install ${PHP_Info[$((PHPSelect-1))]}"
    MPHP_Version=${PHP_Info[$((PHPSelect-1))]#PHP }
    MPHP_Path="/usr/local/php${MPHP_Version%.*}"
...
EOF
    ...
    if [ -s ${MPHP_Path}/sbin/php-fpm ] && [ -s ${MPHP_Path}/etc/php.ini ] && [ -s ${MPHP_Path}/bin/php ]; then
        ...
        rm -rf ${MPHP_Path}
        Echo_Red "Failed to install ${Php_Ver}, see /root/install-mphp8.3.log."
    fi
}
...
EOF
    ...
    if [ -s ${MPHP_Path}/sbin/php-fpm ] && [ -s ${MPHP_Path}/etc/php.ini ] && [ -s ${MPHP_Path}/bin/php ]; then
        ...
        rm -rf ${MPHP_Path}
        Echo_Red "Failed to install ${Php_Ver}, see /root/install-mphp8.4.log."
    fi
}
...
EOF
    ...
    echo "Copy php-fpm init.d file..."
    \cp ${cur_dir}/src/${Php_Ver}/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm8.5
    chmod +x /etc/init.d/php-fpm8.5
    sed -i "s@# Provides:          php-fpm@# Provides:          php-fpm8.5@g" /etc/init.d/php-fpm8.5

    StartUp php-fpm8.5

    \cp ${cur_dir}/conf/enable-php8.5.conf /usr/local/nginx/conf/enable-php8.5.conf
    ...
    if [ -s ${MPHP_Path}/sbin/php-fpm ] && [ -s ${MPHP_Path}/etc/php.ini ] && [ -s ${MPHP_Path}/bin/php ]; then
        ...
        rm -rf ${MPHP_Path}
        Echo_Red "Failed to install ${Php_Ver}, see /root/install-mphp8.5.log."
    fi
}
