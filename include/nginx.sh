...
Install_Nginx_Openssl()
{
    if [ "${Enable_Nginx_Openssl}" = 'y' ]; then
        Download_Files ${Download_Mirror}/lib/openssl/${Openssl_nginx}.tar.gz ${Openssl_nginx}.tar.gz
        [[ -d "${Openssl_nginx}" ]] && rm -rf ${Openssl_nginx}
        tar zxf ${Openssl_nginx}.tar.gz
        Nginx_With_Openssl="--with-openssl=${cur_dir}/src/${Openssl_nginx} --with-openssl-opt='enable-weak-ssl-ciphers'"
    fi
}
...
Install_Ngx_FancyIndex()
{
    if [ "${Enable_Ngx_FancyIndex}" = 'y' ]; then
        ...
        cd ${cur_dir}/src
        Download_Files ${Download_Mirror}/nginx/${NgxFancyIndex_Ver}.tar.xz ${NgxFancyIndex_Ver}.tar.xz
