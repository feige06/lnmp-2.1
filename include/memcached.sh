...
Install_PHPMemcache()
{
    ...
    if echo "${Cur_PHP_Version}" | grep -Eqi '^8.';then
        Download_Files ${Download_Mirror}/memcache/${PHP8Memcache_Ver}.tgz ${PHP8Memcache_Ver}.tgz
        Tar_Cd ${PHP8Memcache_Ver}.tgz ${PHP8Memcache_Ver}
    elif echo "${Cur_PHP_Version}" | grep -Eqi '^7.';then
        Download_Files ${Download_Mirror}/memcache/${PHP7Memcache_Ver}.tgz ${PHP7Memcache_Ver}.tgz
        Tar_Cd ${PHP7Memcache_Ver}.tgz ${PHP7Memcache_Ver}
    else
        if ! gcc -dumpversion|grep -q "^[34]."; then
            export CFLAGS=" -fgnu89-inline"
        fi
        Download_Files ${Download_Mirror}/memcache/${PHPMemcache_Ver}.tgz ${PHPMemcache_Ver}.tgz
...
Install_PHPMemcached()
{
    ...
    Download_Files ${Download_Mirror}/libmemcached/${Libmemcached_Ver}.tar.gz
    ...
    if echo "${Cur_PHP_Version}" | grep -Eqi '^8.';then
        ...
        Download_Files ${Download_Mirror}/php-memcached/${PHP8Memcached_Ver}.tgz ${PHP8Memcached_Ver}.tgz
        Tar_Cd ${PHP8Memcached_Ver}.tgz ${PHP8Memcached_Ver}
    elif echo "${Cur_PHP_Version}" | grep -Eqi '^7.';then
        [[ -d "${PHP7Memcached_Ver}" ]] && rm -rf "${PHP7Memcached_Ver}"
        Download_Files ${Download_Mirror}/php-memcached/${PHP7Memcached_Ver}.tgz ${PHP7Memcached_Ver}.tgz
        Tar_Cd ${PHP7Memcached_Ver}.tgz ${PHP7Memcached_Ver}
    else
        [[ -d "${PHPMemcached_Ver}" ]] && rm -rf "${PHPMemcached_Ver}"
        Download_Files ${Download_Mirror}/php-memcached/${PHPMemcached_Ver}.tgz ${PHPMemcached_Ver}.tgz
...
EOF
    ...
    if [ -s /usr/local/memcached/bin/memcached ]; then
        ...
        Download_Files ${Download_Mirror}/memcached/${Memcached_Ver}.tar.gz ${Memcached_Ver}.tar.gz
