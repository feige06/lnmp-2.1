...
Deb_RemoveAMP()
{
    ...
    if [[ "${DBSelect}" != "0" ]]; then
        if echo "${Ubuntu_Version}" | grep -Eqi "^2[0-7]\."; then
            ...
            for removepackages in mysql-server;
            do apt-get purge -y $removepackages; done
        else
            dpkg -l |grep mysql
    ...
    if [ "${DISTRO}" = "RHEL" ]; then
        if subscription-manager status; then
            ...
            if echo "${RHEL_Version}" | grep -Eqi "^[89]"; then
                subscription-manager repos --enable codeready-builder-for-rhel-${RHEL_Version}-x86_64-rpms
            fi
        else
            RHEL_Modify_Source
        fi
...
Check_Download()
{
    ...
    if [[ "${DBSelect}" =~ ^[1-5]$ ]]; then
        if [[ "${Bin}" = "y" && "${DBSelect}" =~ ^[1-3]$ ]]; then
            Mysql_Ver_Short=$(echo ${Mysql_Ver} | sed 's/mysql-//' | cut -d. -f1-2)
            Download_Files https://cdn.mysql.com/Downloads/MySQL-${Mysql_Ver_Short}/${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz
            if [ $? -ne 0 ]; then
                Download_Files https://cdn.mysql.com/archives/mysql-${Mysql_Ver_Short}/${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz
            fi
        elif [[ "${Bin}" = "y" && "${DBSelect}" = "4" ]]; then
            Download_Files https://cdn.mysql.com/Downloads/MySQL-8.0/${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz
            if [ $? -ne 0 ]; then
                Download_Files https://cdn.mysql.com/archives/mysql-8.0/${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz
            fi
        elif [[ "${Bin}" = "y" && "${DBSelect}" = "5" ]]; then
            Download_Files https://cdn.mysql.com/Downloads/MySQL-8.4/${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz ${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz
            if [ $? -ne 0 ]; then
                Download_Files https://cdn.mysql.com/archives/mysql-8.4/${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz ${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz
            fi
        else
...
Install_Curl()
{
    if [[ ! -s /usr/local/curl/bin/curl || ! -s /usr/local/curl/lib/libcurl.so || ! -s /usr/local/curl/include/curl/curl.h ]]; then
        ...
        if [ -s /usr/local/openssl1.0.2u/bin/openssl ] || /usr/local/openssl1.0.2u/bin/openssl version | grep -Eqi 'OpenSSL 1.0.2'; then
            ./configure --prefix=/usr/local/curl --enable-ares --without-nss --with-zlib --with-ssl=/usr/local/openssl1.0.2u
        else
...
Download_Boost()
{
    ...
    elif [[ "${DBSelect}" =~ ^[45]$ ]] || echo "${mysql_version}" | grep -Eqi '^8.'; then
...
Install_Openssl()
{
    local Openssl_Source="$1"
    if [ ! -s /usr/local/openssl1.0.2u/bin/openssl ] || /usr/local/openssl1.0.2u/bin/openssl version | grep -vq "OpenSSL ${Openssl_Source#openssl-}"; then
        Echo_Blue "[+] Installing ${Openssl_Source}"
        cd ${cur_dir}/src
        Download_Files ${Download_Mirror}/lib/openssl/${Openssl_Source}.tar.gz ${Openssl_Source}.tar.gz
        [[ -d "${Openssl_Source}" ]] && rm -rf ${Openssl_Source}
        Tar_Cd ${Openssl_Source}.tar.gz ${Openssl_Source}
        ./config -fPIC --prefix=/usr/local/openssl1.0.2u --openssldir=/usr/local/openssl1.0.2u
        make depend
        Make_Install
        cd ${cur_dir}/src/
        rm -rf ${cur_dir}/src/${Openssl_Source}
    fi
}

Install_Openssl_New()
{
    local Openssl_Source="${1:-${Openssl_modern}}"
    if [ ! -s /usr/local/openssl1.1.1w/bin/openssl ] || /usr/local/openssl1.1.1w/bin/openssl version | grep -vq "OpenSSL ${Openssl_Source#openssl-}"; then
        Echo_Blue "[+] Installing ${Openssl_Source}"
        cd ${cur_dir}/src
        Download_Files ${Download_Mirror}/lib/openssl/${Openssl_Source}.tar.gz ${Openssl_Source}.tar.gz
        [[ -d "${Openssl_Source}" ]] && rm -rf ${Openssl_Source}
        Tar_Cd ${Openssl_Source}.tar.gz ${Openssl_Source}
        ./config enable-weak-ssl-ciphers -fPIC --prefix=/usr/local/openssl1.1.1w --openssldir=/usr/local/openssl1.1.1w
        make depend
        Make_Install
        cd ${cur_dir}/src/
        rm -rf ${cur_dir}/src/${Openssl_Source}
...
{
    local Openssl_Source="$1"
    if [ ! -s /usr/local/openssl3.5.7/bin/openssl ] || /usr/local/openssl3.5.7/bin/openssl version | grep -vq "OpenSSL ${Openssl_Source#openssl-}"; then
        Echo_Blue "[+] Installing ${Openssl_Source}"
        cd ${cur_dir}/src
        Download_Files https://www.openssl.org/source/${Openssl_Source}.tar.gz ${Openssl_Source}.tar.gz
        [[ -d "${Openssl_Source}" ]] && rm -rf ${Openssl_Source}
        Tar_Cd ${Openssl_Source}.tar.gz ${Openssl_Source}
        ./config -fPIC --prefix=/usr/local/openssl3.5.7 --openssldir=/usr/local/openssl3.5.7
        make depend
        Make_Install
        cd ${cur_dir}/src/
        rm -rf ${cur_dir}/src/${Openssl_Source}
