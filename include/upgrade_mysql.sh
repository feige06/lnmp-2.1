...
Upgrade_MySQL56()
{
    if [ "${Bin}" = "y" ]; then
        ...
        if [ "${isOpenSSL3}" = "y" ]; then
            Install_Openssl_New
            MySQL_WITH_SSL='-DWITH_SSL=/usr/local/openssl1.1.1w'
        else
            MySQL_WITH_SSL=''
        fi
        Tar_Cd mysql-${mysql_version}.tar.gz mysql-${mysql_version}
...
Upgrade_MySQL84()
{
    if [ "${Bin}" = "y" ]; then
        ...
        Tar_Cd ${mysql_src}
        mkdir /usr/local/mysql
        mv mysql-${mysql_version}-linux-glibc2.17-x86_64/* /usr/local/mysql/
    else
        Echo_Blue "Starting upgrade MySQL ${mysql_version} Using Source code..."
        Tar_Cd ${mysql_src} mysql-${mysql_version}
        Install_Boost
...
Upgrade_MySQL()
{
    ...
    if echo "${mysql_version}" | grep -Eqi '^5.[5-7].|^8.';then
    ...
    *)
    ...
    if [[ ! "${mysql_short_version}" =~ ^(5.5|5.6|5.7|8.0|8.4)$ ]]; then
        Echo_Red "MySQL ${mysql_short_version} is not supported."
        exit 1
    fi
    ...
    if [[ "${Bin}" = "y" && "${mysql_short_version}" = "8.0" ]]; then
        mysql_src="mysql-${mysql_version}-linux-glibc2.12-x86_64.tar.xz"
    elif [[ "${Bin}" = "y" && "${mysql_short_version}" = "8.4" ]]; then
        mysql_src="mysql-${mysql_version}-linux-glibc2.17-x86_64.tar.xz"
    elif [[ "${Bin}" = "y" && "${mysql_short_version}" =~ ^5.[5-7]$ ]]; then
        mysql_src="mysql-${mysql_version}-linux-glibc2.12-x86_64.tar.gz"
    else
    ...
    Backup_MySQL
    if [ "${mysql_short_version}" = "5.5" ]; then
        Upgrade_MySQL55
