...
EOF
    ...
    elif [[ "${DBSelect}" =~ ^[45]$ ]] || echo "${mysql_version}" | grep -Eqi '^8.0.'; then
...
Install_MySQL_55()
{
    if [ "${Bin}" = "y" ]; then
        ...
        Tar_Cd ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.gz
        mkdir /usr/local/mysql
        mv ${Mysql_Ver}-linux-glibc2.12-x86_64/* /usr/local/mysql/
    else
...
Install_MySQL_80()
{
    ...
    if [ "${Bin}" = "y" ]; then
        Echo_Blue "[+] Installing ${Mysql_Ver} Using Generic Binaries..."
        Tar_Cd ${Mysql_Ver}-linux-glibc2.12-x86_64.tar.xz
        mkdir /usr/local/mysql
        mv ${Mysql_Ver}-linux-glibc2.12-x86_64/* /usr/local/mysql/
    else
        Echo_Blue "[+] Installing ${Mysql_Ver} Using Source code..."
...
Install_MySQL_84()
{
    ...
    if [ "${Bin}" = "y" ]; then
        Echo_Blue "[+] Installing ${Mysql_Ver} Using Generic Binaries..."
        Tar_Cd ${Mysql_Ver}-linux-glibc2.17-x86_64.tar.xz
        mkdir /usr/local/mysql
        mv ${Mysql_Ver}-linux-glibc2.17-x86_64/* /usr/local/mysql/
    else
        Echo_Blue "[+] Installing ${Mysql_Ver} Using Source code..."
        Tar_Cd ${Mysql_Ver}.tar.gz ${Mysql_Ver}
        Install_Boost
