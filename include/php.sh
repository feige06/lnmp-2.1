...
PHP_with_openssl()
{
    PHP_OpenSSL_Version="${Php_Ver#php-}"
    [[ -z "${PHP_OpenSSL_Version}" || "${PHP_OpenSSL_Version}" = "${Php_Ver}" ]] && PHP_OpenSSL_Version="${php_version}"
    UseOldOpenssl='n'

    case "${PHP_OpenSSL_Version}" in
    5.3.*) PHP_Openssl_Source="${Openssl_php53}" ;;
    5.4.*) PHP_Openssl_Source="${Openssl_php54}" ;;
    5.5.*) PHP_Openssl_Source="${Openssl_php55}" ;;
    5.6.*) PHP_Openssl_Source="${Openssl_php56}" ;;
    7.0.*) PHP_Openssl_Source="${Openssl_php70}" ;;
    7.1.*) PHP_Openssl_Source="${Openssl_php71}" ;;
    7.2.*) PHP_Openssl_Source="${Openssl_php72}" ;;
    7.3.*) PHP_Openssl_Source="${Openssl_php73}" ;;
    7.4.*) PHP_Openssl_Source="${Openssl_php74}" ;;
    8.0.*) PHP_Openssl_Source="${Openssl_php80}" ;;
    8.1.*) PHP_Openssl_Source="${Openssl_php81}" ;;
    8.2.*) PHP_Openssl_Source="${Openssl_php82}" ;;
    8.3.*) PHP_Openssl_Source="${Openssl_php83}" ;;
    8.4.*) PHP_Openssl_Source="${Openssl_php84}" ;;
    8.5.*) PHP_Openssl_Source="${Openssl_php85}" ;;
    *)
        Echo_Red "Unsupported PHP version for OpenSSL: ${PHP_OpenSSL_Version}"
        exit 1
        ;;
    esac

    case "${PHP_Openssl_Source#openssl-}" in
    1.0.2*)
        Install_Openssl "${PHP_Openssl_Source}"
        UseOldOpenssl='y'
        isOpenSSL3='n'
        export PKG_CONFIG_PATH="/usr/local/openssl1.0.2u/lib/pkgconfig:${PKG_CONFIG_PATH}"
        with_openssl='--with-openssl=/usr/local/openssl1.0.2u'
        ;;
    1.1.1*)
        Install_Openssl_New "${PHP_Openssl_Source}"
        isOpenSSL3='n'
        export PKG_CONFIG_PATH="/usr/local/openssl1.1.1w/lib/pkgconfig:${PKG_CONFIG_PATH}"
        with_openssl='--with-openssl=/usr/local/openssl1.1.1w'
        ;;
    3.5.*)
        Install_Openssl_LTS "${PHP_Openssl_Source}"
        isOpenSSL3='y'
        export PKG_CONFIG_PATH="/usr/local/openssl3.5.7/lib/pkgconfig:${PKG_CONFIG_PATH}"
        with_openssl='--with-openssl=/usr/local/openssl3.5.7'
        ;;
    esac
}
...
PHP_with_Sodium()
{
    if [ "${Enable_PHP_Sodium}" = "n" ]; then
        ...
        if echo "${PHPSelect}" | grep -Eqi "^[7-9]|1[0-5]$" || echo "${php_version}" | grep -Eqi '^7.[2-4].*|8.*' || echo "${Php_Ver}" | grep -Eqi "php-7.[2-4].*|php-8.*"; then
...
PHP_with_Intl()
{
    if echo "${Ubuntu_Version}" | grep -Eqi "^19|2[0-7]\." || echo "${Debian_Version}" | grep -Eqi "^1[0-9]" || echo "${Raspbian_Version}" | grep -Eqi "^1[0-9]" || echo "${Deepin_Version}" | grep -Eqi "^2[0-9]" || echo "${UOS_Version}" | grep -Eqi "^2[0-9]" || echo "${Amazon_Version}" | grep -Eqi "^202[3-9]" || echo "${Kali_Version}" | grep -Eqi "^202[2-9]" || echo "${Fedora_Version}" | grep -Eqi "^3[7-9]|4[0-9]" || echo "${openEuler_Version}" | grep -Eqi "^2[2-9]" || echo "${Mint_Version}" | grep -Eqi "^2[0-9]" || echo "${Kylin_Version}" | grep -Eq "^v1[0-9]"; then
        if echo "${PHPSelect}" | grep -Eqi '^[2-5]'  || echo "${php_version}" | grep -Eqi '^5.[4-6].*|7.0.*' || echo "${Php_Ver}" | grep -Eqi "php-5.[4-6].*|php-7.0.*"; then
            Install_Icu60
...
Install_Composer()
{
    if [ "${CheckMirror}" != "n" ]; then
        ...
        if echo "${PHPSelect}" | grep -Eqi '^[1-7]' || echo "${php_version}" | grep -Eqi '^5.[2-6].*|7.[0-2].*' || echo "${Php_Ver}" | grep -Eqi "php-5.[2-6].*|php-7.[0-2].*"; then
            wget --progress=dot:giga --prefer-family=IPv4 --no-check-certificate -T 120 -t3 ${Download_Mirror}/php/composer/composer-2.2.phar -O /usr/local/bin/composer
            if [ $? -eq 0 ]; then
...
Install_PHP_53()
{
    ...
    cd ${cur_dir}/src
        ...
        Download_Files ${Download_Mirror}/zend/ZendGuardLoader-php-5.3-linux-glibc23-${ARCH}.tar.gz ZendGuardLoader-php-5.3-linux-glibc23-${ARCH}.tar.gz
...
Install_PHP_54()
{
    ...
    cd ${cur_dir}/src
        ...
        Download_Files ${Download_Mirror}/zend/ZendGuardLoader-70429-PHP-5.4-linux-glibc23-${ARCH}.tar.gz ZendGuardLoader-70429-PHP-5.4-linux-glibc23-${ARCH}.tar.gz
...
Install_PHP_55()
{
    ...
    cd ${cur_dir}/src
        ...
        Download_Files ${Download_Mirror}/zend/zend-loader-php5.5-linux-${ARCH}.tar.gz zend-loader-php5.5-linux-${ARCH}.tar.gz
...
Install_PHP_56()
{
    ...
    cd ${cur_dir}/src
        ...
        Download_Files ${Download_Mirror}/zend/zend-loader-php5.6-linux-${ARCH}.tar.gz zend-loader-php5.6-linux-${ARCH}.tar.gz
