...
Install_Old_Opcache()
{
    ...
    Download_Files ${Download_Mirror}/opcache/${ZendOpcache_Ver}.tgz ${ZendOpcache_Ver}.tgz
...
Install_Opcache()
{
    ...
    if echo "${Cur_PHP_Version}" | grep -Eqi '^5.[34].'; then
        if [ -s "${zend_ext}" ]; then
            rm -f "${zend_ext}"
        fi
    fi

    if echo "${Cur_PHP_Version}" | grep -Eqi '^5.3.'; then
