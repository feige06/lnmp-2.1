...
Install_Apcu()
{
    ...
    if echo "${Cur_PHP_Version}" | grep -Eqi '^7.|8.'; then
        Download_Files ${Download_Mirror}/apcu/${PHPNewApcu_Ver}.tgz ${PHPNewApcu_Ver}.tgz
        Tar_Cd ${PHPNewApcu_Ver}.tgz ${PHPNewApcu_Ver}
    else
        Download_Files ${Download_Mirror}/apcu/${PHPOldApcu_Ver}.tgz ${PHPOldApcu_Ver}.tgz
    ...
    if echo "${Cur_PHP_Version}" | grep -Eqi '^7.'; then
        Download_Files ${Download_Mirror}/apcu_bc/${PHPApcu_Bc_Ver}.tgz ${PHPApcu_Bc_Ver}.tgz
